...
- 初始化仓库并提交：
```bash
cd /home/kemove/Downloads/subtask_test
git init
git add README.md csv_to_json.py check_videos_exist.py
git commit -m "Initial commit: scripts and README"
```
- 在 GitHub 上新建仓库（例如 `subtask_test`），然后添加远程并推送：
```bash
git branch -M main
...
git push
```
- 首次推送时如提示登录，使用 GitHub 账号或个人访问令牌（PAT）。
